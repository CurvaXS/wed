...
const verifyOtp = async () => {
    ...
    try {
        ...
        if (error.errors && error.errors.otp && error.errors.otp.length > 0) {
            otpError.value = error.errors.otp[0]; // Берем первое сообщение об ошибке
        } else {
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (result.token) {
            ...
            // Update auth store with the full user data from the response
            authStore.token = result.token;
            authStore.user = result.user;
            
            // Set username based on user type and available data
            let username;
            if (result.user.user_type === 'couple' && result.user.couple_profile) {
                // Use the names from the couple profile if available
                username = `${result.user.first_name} и ${result.user.couple_profile.partner_first_name}`;
            } else {
                // Fallback to the form data
                username = form.value.role === 'couple'
                    ? `${form.value.firstName} и ${form.value.partnerFirstName}`
                    : `${form.value.firstName} ${form.value.lastName}`;
            }
            
            // Save username in localStorage and auth store
...
const onOtpInput = (e, index) => {
    ...
    if (value) {
        otpDigits.value[index] = value.slice(-1); // Take only the last character
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-center space-x-2">
                                <input
                                    ...
                                    :class="{ 'border-pink-500': otpDigits[i-1] }"
                                />
                            </div>
